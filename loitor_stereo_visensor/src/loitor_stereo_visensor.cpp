...
void* imu_data_stream(void *)
{
	...
	while((!visensor_Close_IMU_viewer)&&!imu_start_transfer){
		usleep(1000);
		// cout << "IMU hasn't started transferring" << endl;
	}
	while(!visensor_Close_IMU_viewer)
	{
		...
		usleep(10);
		// cout << "IMU started transferring but does not have fresh data" << endl;
	}
	pthread_exit(NULL);
}
...
{ 
	...
	ros::NodeHandle pn("~");
	string imu_port;
	pn.param<string>("imu_port_name", imu_port, "None");

	char imu_port_c[20];
	strcpy(imu_port_c, imu_port.c_str());
	visensor_set_imu_portname(imu_port_c);

	pn.deleteParam("imu_port_name");

	/************************ Start Cameras ************************/

	string this_path = ros::package::getPath("loitor_stereo_visensor");
	string setups_file = "/Loitor_VISensor_Setups.txt";
	stringstream ss;
	ss << this_path << setups_file;

	// if(argv[1])visensor_load_settings(argv[1]);
	// else visensor_load_settings("Loitor_VISensor_Setups.txt");

	visensor_load_settings(ss.str().c_str());
	...
	cout << "Creating IMU data stream thread" << endl;
	...
	cout << "IMU data stream thread created" << endl;
	...
	image_transport::Publisher pub = it.advertise("/camLeft/image_raw", 1);
	sensor_msgs::ImagePtr msg;

	image_transport::ImageTransport it1(n);
	image_transport::Publisher pub1 = it1.advertise("/camRight/image_raw", 1);
	...
	{
		cout << "Joining IMU data stream thread" << endl;
		pthread_join(imu_data_thread,NULL);
		cout << "IMU data stream thread joined" << endl;
	}
	...
	cout << "Cameras shut down" << endl;
	
	cout << "shutting down IMU" << endl;
	/* close IMU */
	visensor_Close_IMU();
	
	cout << "IMU shut down" << endl;
