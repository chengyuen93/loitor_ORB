...
int visensor_get_imu_frame(int fd, unsigned char* imu_frame)
{
    ...
    float fTimeuse;
    // printf("%s%d\n", "SHUT_DOWN_IMU: ", shut_down_imu);
    while(!shut_down_imu)
    {
        // printf("%s%d\n", "IMU_FRAME_LEN: ", IMU_FRAME_LEN);
        //读到32个以上字节
        ...
        // printf("%s%d%s%d\n", "checksum: ", checksum, "\tchecksum in list: ", imu_frame_buf[position_55aa+31]);
        ...
        if(valid_len>32)
        {
            // printf("%s%d\n", "imu_frame_buf[position_55aa + 32]: ", imu_frame_buf[position_55aa + 32]);
            tcflush(fd,TCIFLUSH);
            // memset(imu_frame_buf,0,2*IMU_FRAME_LEN);
            num_get = 0;
            // continue;
